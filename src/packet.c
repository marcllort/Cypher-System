...
{
    //IO_write(1, "PETOOO3", sizeof("PETOOO2"));
    PACKET_destroy(&pd);
    pd = PACKET_create("0", strlen("[TR_caca]"), "[TR_caca]", strlen("test"), "test");
    /*Packet pd;
    pd.type = '0';
    //pd.header = "[TR_caca]";
    strcpy(pd.header,"[TR_caca]");
    strcpy(pd.data,"message");
    //pd.data = "message";
    pd.length = UTILS_sizeOf(pd.data);*/
